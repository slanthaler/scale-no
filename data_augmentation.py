...
class RandomCropResizeTime:
    ...
    def get_params(self, x, re=1, rate=None):
        """Get parameters for ``crop`` for a random sized crop.

        Args:
            x,y (tensor): Input and corresponding output (assumed having same dimensions).

        Returns:
            tuple: params (i, j, k, h, w, t) to be passed to ``crop`` for a random sized crop.
        """
        # Get spatial and temporal dimensions
        T, X, Y = x.shape[-3], x.shape[-2], x.shape[-1]
        assert X >= self.size_min and Y >= self.size_min and T >= self.size_min, \
            f'Cropping only allowed if dimensions >= {self.size_min}. Got X={X}, Y={Y}, T={T}'

        size_min = max(self.size_min, int(round(self.scale_min * min(X, Y, T))))

        if rate is None:
            rnd = torch.rand(1)
            # Calculate crop sizes maintaining aspect ratios
            t_size = size_min + int((T - size_min) * rnd)
            t_size = t_size//2*2
            ...
            # x_size = size_min + int((X - size_min) * rnd)
            # y_size = size_min + int((Y - size_min) * rnd)
            
        else:
            if rate < 1:
                rate = 1/rate
            x_size = int(max(size_min, X // rate))
            y_size = int(max(size_min, Y // rate))
            t_size = int(max(size_min, T // rate))
            scale = ((x_size/X) * (y_size/Y) * (t_size/T)) ** (1/3)

        # Calculate scale based on volume ratio
        # scale = ((x_size/X) * (y_size/Y) * (t_size/T)) ** (1/3)
        re = re * scale

        # Calculate random starting positions
        i = torch.randint(0, X - x_size + 1, size=(1,)).item()
        j = torch.randint(0, Y - y_size + 1, size=(1,)).item()
        k = torch.randint(0, T - t_size + 1, size=(1,)).item()

        return k, i, j, t_size, x_size, y_size, re

    def crop(self, x, i, j, k, h, w, t):
        return x[..., i:i+h, j:j+w, k:k+t]
