...
class CNO2d(nn.Module):
    def __init__(self,
        ...
        self.N_Fourier_F = N_Fourier_F
        self.in_dim = self.in_dim + 2*self.N_Fourier_F

        if self.N_Fourier_F > 0:
            self.FF = FourierFeatures(1, self.N_Fourier_F)
        ...
        for i in range(1, self.N_layers):
            self.decoder_features_in[i] = 2 * self.decoder_features_in[
                i]  # Pad the outputs of the resnets (we must multiply by 2 then)
    ...
    def forward(self, x, re=None):
        ...
        if self.N_Fourier_F > 0:
            grid = self.get_grid()
            ff_grid = self.FF(grid).to(x.device)
            ff_grid = ff_grid.permute(2, 0, 1)
            ff_grid = ff_grid.unsqueeze(0).expand(x.shape[0], -1, -1, -1)
            x = torch.cat((x, ff_grid), 1)
        ...
        x = self.project(x)
        if data_size != self.size:
            x = F.interpolate(x, size=(data_size, data_size))
    ...
    def get_grid(self):
        x = torch.linspace(0, 1, self.size)
        y = torch.linspace(0, 1, self.size)
