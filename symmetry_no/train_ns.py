...
def main(config):
    ...
    if config.dataset == 'darcy':
        ...
        data = NSData(config)
        n_train = config.n_train * data.train_data.n_windows
        n_test = config.n_test * data.train_data.n_windows
        in_channel = 5
    ...
    x, y, re = first_batch['train']
    x, y, re = x.to(device), y.to(device), re.to(device)
    print(f"Input shape: {x.shape}, Target shape: {y.shape}, Reynolds number shape: {re.shape}")
    out = model(x, re)
    ...
    if args.wandb:
        ...
        save_file(model.state_dict(), "NS_model.safetensors")
        wandb.save('NS_model.safetensors')
