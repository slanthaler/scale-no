...
def main(config):
    ...
    # Only get mlp config if using FNO_u or FNO_re models
    mlp = config.mlp if hasattr(config, 'mlp') and config.model in ["FNO_u", "FNO_re"] else None
    ...
    if config.model == "Unet" or config.model == "UNet":
        ...
        model = UNO(
            ...
            n_layers=len(config.uno_out_channels),
            domain_padding=5,
    ...
    for epoch in range(0, epochs + 1):  # config.epochs
        ...
        for d in data.train_loader:
            ...
            for j in range(augmentation_samples):
                if use_augmentation:
                    if config.model == "UNO":
                        loss_aug = LossSelfconsistency(model, x, loss_fn, y=y, type=config.dataset)
                    else:
                ...
                if sample_virtual_instance and (epoch >= start_selfcon):
                    ...
                    new_re = re * rate
                    if config.model == "UNO":
                        loss_sc = LossSelfconsistency(model, new_x, loss_fn, type=config.dataset)
                    else:
