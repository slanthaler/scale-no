...
class UNet2d(nn.Module):
    def __init__(self, in_dim, out_dim, latent_size=128):
        ...
        self.size = (latent_size, latent_size)

    def forward(self, x, re=None):
        # Get the height and width of the input tensor
        input_height = x.shape[-2]  # Second-to-last dimension (height)
        input_width = x.shape[-1]   # Last dimension (width)

        # Check if the input size matches the expected size (self.size)
        if (input_height != self.size[0]) or (input_width != self.size[1]):
            # Resize the input to match self.size (height, width)
            x = F.interpolate(x, size=self.size)

        # Pass the input through the network
        x = self.net(x)

        # Resize the output back to the original input size
        if (input_height != self.size[0]) or (input_width != self.size[1]):
