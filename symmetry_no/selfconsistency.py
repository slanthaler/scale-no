...
from symmetry_no.ns_utilities import NSExtractBC
...
def LossSelfconsistency(model,x,loss_fn,y=None,re=None,rate=None,new_y=None,size_min=32,type="darcy",plot=False,group_action=None,align_corner=False):
    ...
    if type == "darcy":
        ...
        ExtractBD = NSExtractBC
        transform_xy = RandomCropResizeTime(p=1.0, size_min=6)
    elif type == "burgers":
    ...
    if y is not None:
        ...
        if isinstance(transform_xy, RandomCropResizeTime):
            ...
            if type == "NS":
                ...
                data = torch.cat([x[:, 0], y], dim=1) # (B, T, H, W)
                k, i, j, t, h, w, re = transform_xy.get_params(data, re=re, rate=rate)
                # print(f"i: {i}, j: {j}, k: {k}, h: {h}, w: {w}, t: {t}")
                data_small = data[..., k:k+t, i:i+h, j:j+w]
                half_t = t // 2
                
                x_small = data_small[:, :half_t] # (B, half_t, H, W)
                y_small = data_small[:, half_t:] # (B, half_t, H, W)
                bc_small = ExtractBD(y_small) # (B, 5, half_t, H, W)
                x_small = torch.cat([x_small.unsqueeze(1), bc_small], dim=1) # (B, 6, half_t, H, W)
                y_small_ = model(x_small, re) # (B, half_t, H, W)
                
                # min_h = min(y_small.shape[-2], y_small_.shape[-2])
                # min_w = min(y_small.shape[-1], y_small_.shape[-1])
                # y_small = y_small[..., :min_h, :min_w]
                # y_small_ = y_small_[..., :min_h, :min_w]
                
                # y_small [B, 24, h, w]
                # min_t = min(y_small.shape[1], y_small_.shape[1])
                # y_small = y_small[:, :min_t]
                # y_small_ = y_small_[:, :min_t]
            else:
        ...
        if type != "NS":
            ...
            y_small = y_small.reshape(y_small.shape[0], -1)
            y_small_ = y_small_.reshape(y_small_.shape[0], -1)
            return loss_fn(y_small_, y_small)
    ...
    # If y is not given, we set y=model(x). We treat the subdomain as ground truth can detach y_small_
        ...
        if isinstance(transform_xy, RandomCropResizeTime):
            ...
            if type == "NS":
                x_small = x[..., i:i+h, j:j+w, k:k+t]
                y_small = y[..., i:i+h, j:j+w]
