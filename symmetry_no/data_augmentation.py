...
class GridResizing:
    ...
    def forward(self,x,y):
        x_out = GridResize(x,self.grid_size)
        y_out = GridResize(y,self.grid_size)
        x_out = DarcyExtractBC(x_out,y_out)
        return x_out,y_out
...
class RandomFlip:
    ...
    def forward(self,x,y):
        ...
        if torch.rand(1) < self.p:
            x_out = F.hflip(x_out.clone())
            y_out = F.hflip(y_out.clone())
        if torch.rand(1) < self.p:
            x_out = F.vflip(x_out.clone())
            y_out = F.vflip(y_out.clone())
        if torch.rand(1) < self.p:
            x_out = x_out.clone().transpose(-1,-2)
            y_out = y_out.clone().transpose(-1,-2)
            
        x_out = self.ExtractBC(x_out, y_out)
        return x_out, y_out
...
class RandomCropResize:
    def __init__(self, p=0.5, scale_min=0.1, size_min=60):  # Ensure minimum size is 3x modes (3 * 20)
        """
        Args:
...
class RandomCropResizeTime:
    def __init__(self, p=0.5, scale_min=0.1, size_min=40):  # Ensure minimum size is at least 2x modes (2 * 20)
        """
        Args:
            p (float): probability with which to apply the transformation
            scale_min (float): minimal relative scale of subdomain vs. global domain
