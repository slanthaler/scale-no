...
g[:, :, 3] = torch.sin(2*np.pi*4*torch.linspace(1,0,n))
g[:, :, 0] = torch.sin(2*np.pi*4*torch.linspace(0,1,n))

# g = torch.ones((B, n, 4))
g = g.cuda()
u_vectorized = compute_solution_u(g, M=32, N=32)
u_vectorized = u_vectorized.cpu()
