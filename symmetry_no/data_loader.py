...
class NSReader:
    ...
    def __init__(self,
        ...
        # load data
        window_size = T + T_in  # 1 in + 32 pred
        if order == "front":
        ...
        self.n_windows = timesteps // window_size
        
        # [N, n_windows, window_size, S, S]
        data = raw[:, :self.n_windows*window_size].reshape(N, self.n_windows, window_size, raw.shape[2], raw.shape[3])
    ...
    def unpack_mat(self, data, grid_size=None):
        ...
        input_seq = data[:, :, :self.T_in]  # [N, n_windows, T_in, S, S]
        y = data[:, :, self.T_in:self.T_in+self.T]  # [N, n_windows, T, S, S]
        
        # [N*n_windows, T, S, S]
        y = y.reshape(-1, self.T, y.shape[-2], y.shape[-1])
        
        x = NSExtractBC(y)  # [N*n_windows, 5, S, S, T]
