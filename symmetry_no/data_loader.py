...
import h5py
import scipy.io
from typing import List, Union, Dict, Tuple, TypeVar, Generic, TypedDict
...
from symmetry_no.ns_utilities import NSExtractBC, Upsample
...
_T_dict = TypedDict("_T_dict", {"key": _T_co})
...
class NSReader:
    """
    Helper class to read in Navier-Stokes dataset.
    """
    ...
    def __init__(self,
                 ...
                 N=None,
                 order = "front",
                 truncate=None, T=48, sub_s=None, sub_t=None,
                 Re = 1,
                 grid_size=None):
        """
        Args:
            ...
            order (str): "front" or "back" for sample selection
            truncate (int): Truncation factor for spatial dimensions
            T (int): Total window length (must be even, will be split into two halves)
            sub_s (int): Spatial subsampling factor
            sub_t (int): Temporal subsampling factor
            Re (float): Reynolds number
            grid_size (int): Target grid size for resizing
        """
        ...
        # load data
        if order == "front":
            raw = torch.load(self.filepath)[:N]  # [N, 3000, S, S]
        else:
            raw = torch.load(self.filepath)[-N:]

        timesteps = raw.shape[1]
        self.n_windows = timesteps // T
        
        # [N, n_windows, T, S, S]
        data = raw[:, :self.n_windows*T].reshape(N, self.n_windows, T, raw.shape[2], raw.shape[3])
        
        data = data[:, :, ::sub_t, ::sub_s, ::sub_s]
        truncate_S = data.shape[-1] // truncate
        data = data[:, :, :, :truncate_S, :truncate_S]

        self.x, self.y = self.unpack_mat(data, grid_size)

        if Re == None:
            Re = 1000
        self.re = Re * torch.ones(self.x.shape[0] * self.n_windows, 1, requires_grad=False)

    def __len__(self):
        return len(self.x)

    def unpack_mat(self, data, grid_size=None):
        """
        ...
        T_half = self.T // 2
        # full field
        first_half = data[:, :, :T_half]  
        # boundaries
        second_half = data[:, :, T_half:self.T]
        # [N*n_windows, T/2, S, S]
        first_half = first_half.reshape(-1, T_half, first_half.shape[-2], first_half.shape[-1])
        y = second_half.reshape(-1, T_half, second_half.shape[-2], second_half.shape[-1])
        bc = NSExtractBC(y)
        # Create input tensor [N*n_windows, 5, T, S, S]
        # x = torch.zeros(first_half.shape[0], 5, self.T, first_half.shape[2], first_half.shape[3], 
        #                dtype=torch.float32, device=first_half.device)
        # x[:, 0, :T_half] = first_half.permute(0, 1, 2, 3)
        # x[:, :, T_half:] = bc
        # Create input tensor [N*n_windows, 6, T//2, S, S]
        x = torch.cat([first_half.unsqueeze(1), bc], dim=1)
        
        if grid_size and grid_size > 0:
            print("interpolate to", str(grid_size))
            x = Upsample(x, grid_size)
            y = GridResize(y.unsqueeze(1), grid_size).squeeze(1)
